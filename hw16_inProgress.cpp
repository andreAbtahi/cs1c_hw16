...
int main()
{
    ...
    cout << "*      Programmed by Andre Abtahi      * " << endl;
...
// Q#1
// What kinds of operations must type InputIterator and type EqualityComparable support?
// InputIterator are used only on single-pass operations/algorithms.
//EqualityComparable supports only an input iterator that can be compared for equality with another iterator. Just like how types much match.

// Q#5
// What operations must these types have such that copy_if will work?
// InputIterator and OutputIterators work hand in hand with each other
//InputIterator can only access values but not assigned, while OutputIterators can be assign values but cannot access values.
//The Predicate is typically used when there is an InputIterator.
//The predicate accpts functions objects as parameters.
//It takes two values and returns the results. Value_type will allow the templated function to use the same value as the templated parameter.
//Predicates or lambdas allows users to create their own set of rules when working with algorithms, how cool!
